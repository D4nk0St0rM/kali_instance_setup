...
rep -q "^alias ls='ls $LS_OPTIONS'" "${file}" 2>/dev/null || echo "alias ls='ls $LS_OPTIONS'" >> "${file}"
...
if [ -e "/etc/vmware-tools" ]; then
  ...
  sudo apt-get install -y --reinstall open-vm-tools open-vm-tools-desktop open-vm-tools-dkms fuse || echo -e ' '${RED}'[!] Issue when git cloning'${RESET} 1>&2
  sudo apt-get -y -qq install make || echo -e ' '${RED}'[!] Issue with apt-get'${RESET}    # nags afterwards
